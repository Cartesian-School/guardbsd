...
//! - Install exception vectors 0..31 as dedicated ASM stubs (`exc_stub_0..exc_stub_31`)
//!   to avoid triple-fault and help early debugging.
//!
//! Notes:
//! - IST is set to 0 for all entries because boot stub does not yet maintain
//!   per-exception interrupt stacks (IST1..IST7). The early boot environment is
//!   single-stack and we want deterministic, minimal setup.
...
// TODO(arch): move selectors to a shared x86_64 GDT module to avoid drift.
...
// 0xEE = Present | DPL=3 | Interrupt Gate (type=0xE) (user-callable int gate)
const FLAG_INTGATE_DPL0: u8 = 0x8E;
const FLAG_INTGATE_DPL3: u8 = 0xEE;

// PIC remap base vectors (if using classic 8259 PIC mapping to 0x20..0x2F).
...
extern "C" {
    // Assembly entry stubs provided elsewhere (interrupt stubs / boot asm).
    fn keyboard_irq_handler();
    fn timer_irq_handler();
    fn syscall_entry();

    // Optional: per-vector exception stubs (ASM).
    // Enabled only when `feature = "idt_exceptions"` is set.
...
///
/// Safe to call after GDT is loaded and the kernel CS selector (KERNEL_CS) is valid.
pub fn init_idt() {
    unsafe {
        // Optional: install exception vectors 0..31 for early debugging
        // (avoid triple-fault on #GP/#PF/#DF etc.).
        #[cfg(feature = "idt_exceptions")]
        {
        ...
        // IRQ0 timer interrupt (vector 0x20)
        set_idt_entry(
            IRQ0_VECTOR,
            timer_irq_handler as usize as u64,
            KERNEL_CS,
            FLAG_INTGATE_DPL0,
        );

        // IRQ1 keyboard interrupt (vector 0x21)
        set_idt_entry(
            IRQ1_VECTOR,
            keyboard_irq_handler as usize as u64,
            KERNEL_CS,
            FLAG_INTGATE_DPL0,
        );

        // Syscall gate (INT 0x80) - callable from ring3 (DPL=3)
        set_idt_entry(
            SYSCALL_VECTOR,
...
unsafe fn set_idt_entry(index: usize, handler: u64, selector: u16, flags: u8) {
    if index >= IDT_ENTRIES {
        // Boot stub: silently ignore invalid indices (defensive).
        return;
    }
    ...
    IDT[index] = IdtEntry {
        ...
        // This keeps the setup minimal and predictable during early bring-up.
        ist: 0,
        flags,
