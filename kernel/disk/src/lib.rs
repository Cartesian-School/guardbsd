...
pub fn read_block_cached(device_id: u32, block_num: u64, buf: &mut [u8; BLOCK_SIZE]) -> Result<(), DiskError> {
...
pub fn write_block_cached(device_id: u32, block_num: u64, buf: &[u8; BLOCK_SIZE]) -> Result<(), DiskError> {
    if let Some(device) = get_disk(device_id) {
