...
impl ProcessSpawner {
    ...
    fn init_path(&mut self, env: &Environment) {
            ...
            for i in 0..path_value.len() {
                if path_value[i] == b':' || path_value[i] == 0 {
                    if self.path_count < 16 && i > start {
                        ...
                        self.path_dirs[self.path_count][..len].copy_from_slice(&path_value[start..start + len]);
                        self.path_count += 1;
                    }
                    start = i + 1;
                }
        ...
        if self.path_count == 0 {
            self.path_dirs[0] = *b"/bin\0                                                          ";
            self.path_dirs[1] = *b"/usr/bin\0                                                      ";
            self.path_count = 2;
        }
    }
    ...
    pub fn spawn(&self, cmd: &Command, env: &Environment) -> Result<i32> {
        // Find executable
        let exec_path = self.find_executable(cmd.name)
